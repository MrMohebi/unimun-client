...
const MyBooks = () => {
    ...
    useEffect(() => {
        ...
        if (UserToken()) {
            getMyBooks().then(e => {
                try {
                    _myBooks(e.data.booksUserCreated.edges);
        ...
        <div className={'h-full pb-20 '}>
            ...
            <div className={'h-10 w-full bg-white  z-50'}>
                ...
                <Tab indicatorAtBottom={true} indicatorSizeDivider={2} activeIndex={currentActivePart}>
                    ...
                    <div className={`w-20 text-center IranSansMedium ${currentActivePart === 1 ? "text-primary" : ""}`}
                         onClick={() => {
                             _currentActivePart(1)
                         }}>کتاب ها
                    </div>
                    <div className={`w-20 text-center IranSansMedium ${currentActivePart === 2 ? "text-primary" : ""}`}
                         onClick={() => {
            ...
            <div className={'w-full h-full overflow-scroll px-3'} id={'my-books-scroller'}>
                {
                    (currentActivePart === 2 && myBooks.length === booksLength) ?

                        <div className={'h-10 w-full IranSans text-center text-textDark '}>هنوز کتابی اضافه
                            نکردی</div>
                        : <div
                            className={'h-10 w-full IranSans text-center text-textDark '}>{booksLength} - {myBooks.length}</div>
                }
                <InfiniteScroll pullDownToRefreshContent={<h1 className={'h-10'}></h1>} releaseToRefreshContent={<div
                    className={'h-10 w-full text-center IranSans text-sm '}>حله ولش کن :)</div>} className={''}
...

                    {
                        myBooks.map((item, index) => {
                            ...
                            // if (currentActivePart === 1 && !getMyBooksData.loading && booksLength === 0) {
                            //     return (
                            //         <div className={'h-10 w-full IranSans text-center text-textDark '}>هنوز کتابی اضافه
                            //             نکردی</div>
                            //     )
                            ...
                            if (book.isBook && currentActivePart === 1 || !book.isBook && currentActivePart === 2) {
